...
class StatsManager:
    ...
    def get_today_stats(self) -> Dict:
        ...
        if today not in stats["daily"]:
            stats["daily"][today] = {"total": 0, "work": 0, "break": 0}
...
class PomodorTimer:
    ...
    def run(self, session_type: str) -> None:
        ...
        if session_type not in self.SESSIONS:
            ...
            self.console.print(
                f"[cyan]Available: {', '.join(self.SESSIONS.keys())}[/cyan]"
            )
            sys.exit(1)
        ...
        self.console.print(
            f"[cyan]📊 Heute: {today_stats['total']} Sessions ([yellow]{today_stats['work']}[/yellow] Arbeit, [green]{today_stats['break']}[/green] Pause)[/cyan]\n"
        )
                ...
                while not progress.finished:
                    ...
                    if remaining <= 0:
                        progress.update(
                            ...
                            remaining=self.format_time(0),
                        )
                        break

                    progress.update(
                        task, completed=elapsed, remaining=self.format_time(remaining)
                    )
                    time.sleep(0.5)
        ...
        # Session complete
        complete_text = (
            f"{self.EMOJIS['done']}  {session_type.upper()} SESSION ABGESCHLOSSEN!"
        )
        ...
        today_stats = self.stats_manager.get_today_stats()
        self.console.print(
            f"[cyan]📊 Heute insgesamt: {today_stats['total']} Sessions ([yellow]{today_stats['work']}[/yellow] Arbeit, [green]{today_stats['break']}[/green] Pause)[/cyan]\n"
        )
...
Examples:
  ...
  pdoro gym -d 60         Create your own custom session
    ...
    parser.add_argument(
        "-s", "--stats", action="store_true", help="Show usage statistics"
    )
    ...
    if args.list:
        ...
        for session, duration in timer.SESSIONS.items():
            ...
            timer.console.print(
                f"  [cyan]•[/cyan] [yellow]{session}[/yellow]: {duration} minutes {emoji}"
            )
        timer.console.print()
        sys.exit(0)
    ...
    if args.stats:
        ...
        timer.console.print(
            f"    [cyan]Total Sessions:[/cyan] {stats['total_sessions']}"
        )
        timer.console.print(
            f"    [yellow]Work Sessions:[/yellow] {stats.get('work_sessions', 0)}"
        )
        timer.console.print(
            f"    [green]Break Sessions:[/green] {stats.get('break_sessions', 0)}\n"
        )
        sys.exit(0)
    ...
    if args.session is None:
        timer.console.print(
            "[red]❌ Please specify a session or use -l to list available sessions[/red]"
        )
        sys.exit(1)
